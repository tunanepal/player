  ...
  <article class="card room" data-room="${r.id}">
    <div class="room__top">
      ...
      <div class="room__title">
        <h3>${esc(r.team_size.toUpperCase())} · ${esc(GAMES[r.game].short)}</h3>
...
async function openDetails(id) {
  let r;
  try { r = await rpcAuth('tuna_room_details', { p_room: id }); }
  catch (e) { return toast(e.message, 'bad'); }
  if (!r) return toast('This room is no longer listed.', 'bad');

  const yn = (v) => v ? '<b class="yes">Yes</b>' : '<b class="no">No</b>';
  const cells = [];
  cells.push(`<div><small>Players</small><b>${esc(r.team_size.toUpperCase())}</b></div>`);
  cells.push(`<div><small>Game</small><b>${esc(GAMES[r.game].short)}</b></div>`);
  if (r.game === 'pubg') cells.push(`<div><small>Gun type</small><b>${esc(r.gun_type || '—')}</b></div>`);
  const add = (label, v) => { if (v !== null && v !== undefined) cells.push(`<div><small>${label}</small>${yn(v)}</div>`); };
  add('Headshot only', r.headshot);
  add('Limited ammo', r.limited_ammo);
  add('Throwables', r.throwables);
  add('Gun attributes', r.gun_attr);
  add('Character skill', r.char_skill);
  cells.push(`<div><small>Posted</small><b style="font-size:15px">${esc(when(r.created_at))}</b></div>`);

  const pot = r.amount * 2;
  const payout = Math.round(pot * 0.88);

  openSheet(`
    <h2>${esc(r.team_size.toUpperCase())} · ${esc(GAMES[r.game].short)}</h2>
    <p class="sheet__sub">Hosted by ${esc(r.host_name)}</p>
    <div class="alert alert--bad" id="joinErr" hidden></div>

    <div class="spec">${cells.join('')}</div>
    ...
    ${r.is_host
      ...
      : `<button class="btn" id="joinYes">Interested — pay ${money(r.amount)}</button>
         ...
         <p class="xs muted center" style="margin-top:10px">
           Room ID and password unlock the moment you pay in.</p>`}
  `);
...
async function accept(btn, room) {
  ...
  try {
    const out = await busy(btn, 'Joining…', () =>
      rpcAuth('tuna_accept_room', { p_room: room.id }));
    await refreshMe();
    showCreds(out, room);
    loadRooms();
  } catch (e) {
